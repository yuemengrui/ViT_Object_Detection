...
class ViTSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            new_target = cv2.resize(target, (0, 0), fx=scale, fy=scale)

            similarity_matrix = cv2.matchTemplate(img, new_target, cv2.TM_CCOEFF_NORMED)
            similarity_matrix = cv2.resize(similarity_matrix, (832, 512))
            similarity_matrix = np.expand_dims(similarity_matrix, 0)
            ...
            return img, target, similarity_matrix, label
...
if __name__ == '__main__':
    ...
    img, target, mask, label = dataset[0]
    print(img.shape)
    print(target.shape)
    print(mask.shape)
    print(label.shape)
