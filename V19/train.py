...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (img, img_texts, cxs, cys, target, target_texts, label_box) in enumerate(self.train_loader):
            self.global_step += 1
            ...
            preds = self.model(img, img_texts, cxs, cys, target, target_texts)
    ...
    def _eval(self):
        ...
        iou_list = []
        # batch_start = time.time()
        for img, img_texts, cxs, cys, target, target_texts, label_box in self.val_loader:
            with torch.no_grad():
                img = img.to(self.device)
                img_texts = img_texts.to(self.device)
                cxs = cxs.to(self.device)
