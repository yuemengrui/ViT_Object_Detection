...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (img, target, box, label_box) in enumerate(self.train_loader):
            self.global_step += 1
            ...
            pred0, pred1 = self.model(data)
            loss = self.criterion(pred0, pred1, box, label_box)
            # backward
            loss.backward()
            self.optimizer.step()
    ...
    def _eval(self):
        ...
        # batch_start = time.time()
        for img, target, box, label_box in self.val_loader:
            with torch.no_grad():
                img = img.to(self.device)
                target = target.to(self.device)
                box = box.to(self.device)
                ...
                self.metric_cls.update(pred0, pred1, box, label_box)
                # batch_cost = time.time() - batch_start
