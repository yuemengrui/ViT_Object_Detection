...
class ViT(nn.Module):
    ...
    def __init__(self, image_size=(512, 1024), patch_size=(32, 64), dim=512, depth=6, heads=8, mlp_dim=512, channels=3,
        ...
        self.img_transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout)
        self.target_transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout)
    ...
    def forward(self, img, target_img=None):
        x = self.to_patch_embedding(img)  # [N, 256, 512]

        target = self.to_target_embedding(target_img)  # [N, 1, 512]

        b, n, _ = x.shape

        x = self.img_transformer(x)  # [N, 256, 512]

        target = self.target_transformer(target)  # [N, 1, 512]
