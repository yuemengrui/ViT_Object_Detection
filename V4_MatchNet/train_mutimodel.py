...
from dataset_mutimodel import MatchDataset
from metrics import ClassifyMetric
...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (img, img_text_encode, target, target_text_encode, label) in enumerate(self.train_loader):
            self.global_step += 1
    ...
    def _eval(self):
        ...
        # batch_start = time.time()
        for img, img_text_encode, target, target_text_encode, label in self.val_loader:
            with torch.no_grad():
                img = img.to(self.device)
                img_text_encode = img_text_encode.to(self.device)
                target = target.to(self.device)
    ...
    def _initialize(self):
        ...
        self.metric_cls = ClassifyMetric()
        ...
        train_dataset = MatchDataset(self.configs.get('dataset_dir'))
        val_dataset = MatchDataset(self.configs.get('dataset_dir'), mode='val')
...
if __name__ == '__main__':
    configs = {
        'save_dir': './checkpoints_mutimodel',
