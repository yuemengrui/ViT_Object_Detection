...
class ViTSegDataset(Dataset):
    ...
    def _text_encode(self, text, max_length=64):
        text_encode = []
        if text == '':
    ...
    def ocr_res_handler(self, img_h, img_w, h_scale, w_scale):
        ...
        return np.array(img_texts), np.array(img_boxes), np.array(target_texts)
    ...
    def __getitem__(self, idx):
        try:
            ...
            img_texts, img_boxes, target_texts, = self.ocr_res_handler(img_h, img_w, h_scale, w_scale)
            ...
            # return img, target, label_box
            return img, torch.from_numpy(img_texts).float(), torch.from_numpy(
                img_boxes).float(), target, torch.from_numpy(target_texts).float(), label_box
...
if __name__ == '__main__':
    ...
    #     # h, w = img.shape[:2]
    #     # print(label_box)
    #     # box = [int(label_box[0] * w), int(label_box[1] * h), int(label_box[2] * w), int(label_box[3] * h)]
    #     #
    ...
    #
    for img, img_texts, img_boxes, target, target_texts, label_box in train_loader:
        print("====================")
        ...
        s = time.time()
        out = net(img, img_boxes, img_texts, target, target_texts)
        print('out: ', out)
        print(1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(out), box_cxcywh_to_xyxy(label_box))))
        print(time.time() - s)
        #         iou, _ = box_iou(out, label_box)
        #         print(type(iou))
        #         print(type(float(iou)), float(iou))
        #         # print(F.l1_loss(out, label_box))
