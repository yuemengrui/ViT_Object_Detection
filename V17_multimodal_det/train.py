...
from loss import SetCriterion, box_iou, box_cxcywh_to_xyxy
...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (img, img_texts, img_boxes, target, target_texts, label_box) in enumerate(self.train_loader):
            self.global_step += 1
            lr = self.optimizer.param_groups[0]['lr']

            img = img.to(self.device)
            img_texts = img_texts.squeeze(dim=0).to(self.device)
            img_boxes = img_boxes.squeeze(dim=0).to(self.device)
            target = target.to(self.device)
            target_texts = target_texts.squeeze(dim=0).to(self.device)
            label_box = label_box.to(self.device)

            reader_cost = time.time() - batch_start

            cur_batch_size = img.size()[0]

            preds = self.model(img, img_boxes, img_texts, target, target_texts)
            loss = self.criterion(preds, label_box)
            # backward
            ...
            if i % 50 == 0:
                ...
                logger.train.info(
                    '[epoch:{}/{}] [iter:{}/{}] global_step:{}, {} lr:{:.9f}, reader_cost:{:.2f}s, batch_cost:{:.2f}s, speed:{:.1f}/s'.format(
                        epoch, self.epochs, i + 1, self.train_loader_len, self.global_step, loss_str, lr,
                        reader_cost,
    ...
    def _eval(self):
        ...
        iou_list = []
        # batch_start = time.time()
        for img, img_texts, img_boxes, target, target_texts, label_box in self.val_loader:
            with torch.no_grad():
                img = img.to(self.device)
                img_texts = img_texts.squeeze(dim=0).to(self.device)
                img_boxes = img_boxes.squeeze(dim=0).to(self.device)
                ...
                iou, _ = box_iou(box_cxcywh_to_xyxy(preds), box_cxcywh_to_xyxy(label_box))
    ...
    def _initialize(self):
        ...
        # self.lr_scheduler = lr_scheduler.StepLR(optimizer=self.optimizer, step_size=self.configs.get('lr_drop'))
