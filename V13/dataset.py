...
class ViTSegDataset(Dataset):
    ...
    def __init__(self, dataset_dir, mode='train', target_w_min=32, target_h_min=20,
                 target_w_h_rate=(0.8, 5.0), threshold=0.3, **kwargs):
    ...
    def get_crop_img(self, ori_h, ori_w, binary, h_range, w_range):
        start = time.time()
        while True:
            end = time.time()
            if end - start > 12:
    ...
    def create_target(self, targets, origin_img, ori_h, ori_w, binary, target_h_range, target_w_range,
                      origin_point_list):
        ...
        for n in range(target_num):
            ...
            target_box = self.get_crop_img(ori_h, ori_w, binary, target_h_range, target_w_range)
    ...
    def __getitem__(self, idx):
        try:
            ...
            target_w_range = (self.target_w_min, target_w_max)
            target_h_range = (self.target_h_min, target_h_max)

            targets = np.zeros_like(img)

            targets, box_list = self.create_target(targets, img, ori_h, ori_w, binary, target_h_range, target_w_range,
                                                   origin_point_list)
            ...
            for b in box_list:
                label[b[1]:b[3], b[0]:b[2]] = 1

            img = self.image_resize(img)

            targets = self.image_resize(targets)
