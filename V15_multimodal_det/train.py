...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (
            ...
            img_text_lengths = img_text_lengths.to(self.device)
    ...
    def _eval(self):
        ...
        iou_list = []
        # batch_start = time.time()
        for img, img_texts, img_text_lengths, img_boxes, target, target_texts, target_text_lengths, label_box in self.val_loader:
            with torch.no_grad():
                img = img.to(self.device)
                img_texts = img_texts.to(self.device)
                img_text_lengths = img_text_lengths.to(self.device)
