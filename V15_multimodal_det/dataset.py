...
class ViTSegDataset(Dataset):
    ...
    def ocr_res_handler(self, img_h, img_w, h_scale, w_scale):
        ...
        return np.array(img_texts), np.array(img_text_lengths), np.array(img_boxes), np.array(target_texts), np.array(
            target_text_lengths)
    ...
    def __getitem__(self, idx):
        try:
            ...
            label_box = torch.from_numpy(np.array(label_box)).unsqueeze(0)

            # return img, target, label_box
            return img, img_texts, img_text_lengths, img_boxes, target, target_texts, target_text_lengths, label_box
    ...
    def collate(self, samples):
        img, img_texts, img_text_lengths, img_boxes, target, target_texts, target_text_lengths, label_box = map(list,
                                                                                                                zip(*samples))
        img = torch.cat(img, dim=0)
        target = torch.cat(target, dim=0)
        label_box = torch.cat(label_box, dim=0)
        ...
        return img, torch.from_numpy(img_texts), torch.from_numpy(img_text_lengths), torch.from_numpy(
            img_boxes).float(), \
            target, torch.from_numpy(target_texts), torch.from_numpy(target_text_lengths), label_box
